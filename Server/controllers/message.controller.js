...
class MessageController {
  ...
  */
  static async create(request, response) {
    ...
    try {
      const { message, email, subject, status } = request.body;
      let msg = {
        message, email, subject, senderId: request.user.id, status,
      };
      ...
      if (errors.length === 0) {
        ...
        response.send({ status: 201, data: [msg] });
      } else {
        response.send({ status: 400, errors });
      }
    } catch (err) {
  ...
  static async get(request, response) {
    try {
      ...
      if (msg && (msg.senderid === request.user.id || msg.receiverid === request.user.id)) {
        if (msg.receiverid === request.user.id) { await (new Message(msg)).setRead(); }
        response.send({ status: 200, data: [msg] });
      } else {
        response.send({ status: 400, data: ['Invalid input'] });
      }
    } catch (err) {
      response.send({ status: 500, errors: ['Internal Error'] });
    }
